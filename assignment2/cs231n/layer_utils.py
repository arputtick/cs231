...
######## DEFINED MY OWN FUNCTIONS for affine - bn/ln - ReLU ####################
def affine_bn_relu_forward(x, w, b, gamma, beta, bn_param):
    ...
    - bn_param: dictionary containing parameters for batchnorm layer
    ...
    a_2, bn_cache = batchnorm_forward(a_1, gamma, beta, bn_param)
...
def affine_bn_relu_backward(dout, cache):
    ...
    da_1, dgamma, dbeta = batchnorm_backward_alt(da_2, bn_cache)
...
def affine_ln_relu_forward(x, w, b, gamma, beta, bn_param):
    ...
    """
    a_1, fc_cache = affine_forward(x, w, b)
    a_2, ln_cache = layernorm_forward(a_1, gamma, beta, bn_param)
    out, relu_cache = relu_forward(a_2)
...
def affine_ln_relu_backward(dout, cache):
    ...
    da_2 = relu_backward(dout, relu_cache)
    da_1, dgamma, dbeta = layernorm_backward(da_2, ln_cache)
    dx, dw, db = affine_backward(da_1, fc_cache)
    return dx, dw, db, dgamma, dbeta
